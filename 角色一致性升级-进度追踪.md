...
- [ ] **严格阈值回归（非 quick）**：用写回后的阈值跑一轮 high_risk + 若干战斗/遮挡场景，确认“更严格漂移限制(0.05)”不会误伤
- [ ] **场景集扩充 + 数据沉淀**：增加真实生产 prompt 模板（战斗/高速运动/强遮挡/光照极端/多人），并固定为 nightly regression 集
- [ ] **回归自动化**：将“全套回归 + 汇总报告”固化成一条命令（含时间戳目录、自动归档、失败样本拎出）
...
| M6: 视频身份保持 | 1月15日 | ✅ 99% | 回归全绿(28/28) + 阈值落盘 + hard case 可救回（收尾：严格阈值回归/场景扩充/自动化） |
