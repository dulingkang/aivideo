...
process_episode() {
    local EPISODE=$1
    local JSON_FILE="renjie/episode/${EPISODE}.json"
    ...
    # 检查输出文件是否已存在
    OUTPUT_FILE="renjie/episode_${EPISODE}_video.mp4"
    ...
    # 检查必需文件
    if [ ! -f "renjie/opening_video_raw.mp4" ]; then
        echo "❌ 错误: renjie/opening_video_raw.mp4 不存在"
        return 1
    fi
    
    if [ ! -f "renjie/ending_video_raw.mp4" ]; then
        echo "⚠️  警告: renjie/ending_video_raw.mp4 不存在，将跳过结尾"
        ENDING_FLAG=""
    else
        ENDING_FLAG="--ending renjie/ending_video_raw.mp4"
    fi
    ...
    if [ -n "$ENDING_FLAG" ]; then
        CMD_ARGS+=(--ending renjie/ending_video_raw.mp4)
    fi
...
if [ "$PROCESS_ALL" = true ]; then
    ...
    JSON_FILES=($(ls -1 renjie/episode/*.json | sort -V))
    TOTAL=${#JSON_FILES[@]}
    SUCCESS=0
    FAILED=0
    SKIPPED=0
    ...
    for JSON_FILE in "${JSON_FILES[@]}"; do
        # 从文件名提取集数（例如：renjie/episode/171.json -> 171）
        EPISODE_NUM=$(basename "$JSON_FILE" .json)
        
        if [ "$SKIP_EXISTING" = true ] && [ -f "renjie/episode_${EPISODE_NUM}_video.mp4" ]; then
    ...
    echo "✅ 视频生成完成！"
    echo "============================================================"
    echo "输出文件: renjie/episode_${EPISODE}_video.mp4"
