...
1. **显存优化 (Memory Optimization) ✅**
   - **问题**: 原生 Flux 加载导致显存峰值翻倍 (46GB)，超出单卡限制。
   - **方案**: 实现 `_optimized_load_flux`，将权重加载至 CPU 内存再逐层转移至 GPU，避免双重显存占用。
   - **效果**: 峰值显存降低至 ~26GB (Flux+PuLID)，适配 40GB+ 显卡，甚至可尝试 24GB (需更激进 offload)。
...
```
gen_video/
├── pulid_engine.py              # PuLID-FLUX 身份保持引擎 (已集成显存优化)
...
| 模块 | 状态 | 说明 |
|------|------|------|
| `pulid_engine.py` | ✅ 已优化 | PuLID 引擎封装 (Native Mode + VRAM Opt) |
| `decoupled_fusion_engine.py` | ✅ 完成 | 解耦融合实现 |
| `execution_planner_v3.py` | ✅ 完成 | 智能路由系统 |
| `enhanced_image_generator.py` | ✅ 完成 | 整合入口 |
